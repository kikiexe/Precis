<script lang="ts">
  import { Camera, Eye } from 'lucide-svelte';
  import type { AttendanceRecord } from '../../../types/app';
  import AttendanceDetailModal from './modals/AttendanceDetailModal.svelte';

  interface Props {
    attendances: AttendanceRecord[];
    selectedBranchFilter?: string;
  }

  let {
    attendances = [],
    selectedBranchFilter = 'ALL',
  }: Props = $props();

  let selectedAttendanceDetail = $state<AttendanceRecord | null>(null);

  let filteredAttendances = $derived(
    attendances.filter((att) => {
      if (selectedBranchFilter === 'ALL') return true;
      return att.branch_name && att.branch_name.toLowerCase().includes(selectedBranchFilter.toLowerCase());
    })
  );

  let onTimeCount = $derived(filteredAttendances.filter((a) => !a.late_minutes || a.late_minutes === 0).length);
  let lateCount = $derived(filteredAttendances.filter((a) => a.late_minutes && a.late_minutes > 0).length);
</script>

<div class="space-y-6 font-sans">
  <!-- Status Counter Header Strip -->
  <div class="bg-white border border-[#e5e5ea] rounded-2xl sm:rounded-3xl p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xs">
    <div class="space-y-1">
      <h2 class="text-base sm:text-lg font-bold text-[#17171c]">Wall of Faces &amp; Presensi Selfie</h2>
      <p class="text-xs text-[#8e8e93]">
        Verifikasi visual kehadiran staf saat clock-in dengan foto selfie dan deteksi lokasi.
      </p>
    </div>

    <div class="flex items-center gap-3">
      <div class="inline-flex items-center gap-3 px-4 py-2 bg-[#f8f8fa] border border-[#e5e5ea] rounded-full text-xs font-mono">
        <div class="flex items-center gap-1.5 font-semibold text-[#059669]">
          <span class="w-2 h-2 rounded-full bg-[#059669]"></span>
          <span>Tepat Waktu: {onTimeCount}</span>
        </div>
        <span class="text-[#d1d1d6]">&bull;</span>
        <div class="flex items-center gap-1.5 font-semibold text-[#e5484d]">
          <span class="w-2 h-2 rounded-full bg-[#e5484d]"></span>
          <span>Terlambat: {lateCount}</span>
        </div>
      </div>
    </div>
  </div>

  <!-- Presensi Gallery Grid (3:4 Portrait Aspect Ratio) -->
  {#if filteredAttendances.length === 0}
    <div class="bg-white border border-[#e5e5ea] rounded-2xl sm:rounded-3xl p-12 text-center space-y-3 shadow-2xs">
      <div class="w-12 h-12 rounded-2xl bg-[#f4f4f6] text-[#8e8e93] flex items-center justify-center mx-auto">
        <Camera class="w-6 h-6" />
      </div>
      <div>
        <h3 class="text-sm font-bold text-[#17171c]">Belum Ada Presensi Hari Ini</h3>
        <p class="text-xs text-[#8e8e93] mt-1 max-w-sm mx-auto">
          Foto selfie karyawan saat clock-in akan muncul di sini secara realtime.
        </p>
      </div>
    </div>
  {:else}
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3.5 sm:gap-4">
      {#each filteredAttendances as att}
        <button
          type="button"
          onclick={() => (selectedAttendanceDetail = att)}
          class="bg-white border border-[#e5e5ea] hover:border-[#17171c]/40 rounded-2xl overflow-hidden text-left group hover:shadow-md transition-all cursor-pointer block w-full focus:outline-hidden shadow-2xs"
        >
          <!-- 3:4 Aspect Ratio Photo Container with Clean Floating Pill -->
          <div class="aspect-[3/4] bg-[#f4f4f6] relative overflow-hidden">
            {#if att.photo_in_url || att.avatar_url}
              <img
                src={att.photo_in_url || att.avatar_url}
                alt={att.user_name}
                class="w-full h-full object-cover group-hover:scale-105 transition-all duration-300"
              />
            {:else}
              <div class="w-full h-full flex items-center justify-center text-xs font-mono text-[#8e8e93]">
                Tanpa Foto
              </div>
            {/if}

            <!-- Status Badge -->
            <div class="absolute top-2.5 right-2.5">
              {#if !att.late_minutes || att.late_minutes === 0}
                <span class="text-[9.5px] font-mono font-semibold px-2 py-0.5 rounded-full bg-black/60 backdrop-blur-md text-[#34d399] border border-white/20 shadow-xs">
                  Tepat Waktu
                </span>
              {:else}
                <span class="text-[9.5px] font-mono font-semibold px-2 py-0.5 rounded-full bg-black/60 backdrop-blur-md text-[#f87171] border border-white/20 shadow-xs">
                  Telat {att.late_minutes}m
                </span>
              {/if}
            </div>

            <!-- Hover Preview Overlay -->
            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <span class="px-3 py-1.5 bg-white/90 backdrop-blur-xs text-[#17171c] text-xs font-semibold rounded-full shadow-xs flex items-center gap-1.5">
                <Eye class="w-3.5 h-3.5" />
                <span>Lihat Detail</span>
              </span>
            </div>
          </div>

          <!-- Card Bottom Info -->
          <div class="p-3 space-y-1 bg-white">
            <div class="font-bold text-xs text-[#17171c] truncate">{att.user_name}</div>
            <div class="text-[11px] text-[#686873] font-mono flex items-center justify-between">
              <span>{att.branch_name || 'Outlet'}</span>
              <span class="text-[#8e8e93]">{att.clock_in_time ? att.clock_in_time.substring(11, 16) : '-'}</span>
            </div>
          </div>
        </button>
      {/each}
    </div>
  {/if}
</div>

<!-- Attendance Detail Modal -->
{#if selectedAttendanceDetail}
  <AttendanceDetailModal
    attendance={selectedAttendanceDetail}
    onClose={() => (selectedAttendanceDetail = null)}
  />
{/if}
